...
public class Enemy : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        switch (state)
        {
            ...
            case 2:
                if (AnimationObject)
                    AnimationObject.CrossFade(walkPose, 0.3f);
                Vector3 direction_1 = (targetPosition - transform.position).normalized;
                moveDirection = Vector3.Lerp(moveDirection, direction_1, Time.deltaTime * Slip);
                break;
        }
        ...
        if (Physics.Raycast(transform.position, Vector3.down, DistanceToGround, LayerMask))
        {
            isGrounded = true;
        }
        else
